...
let PermissionGuard = class PermissionGuard {
    ...
    canActivate(context) {
        const required = this.reflector.getAllAndOverride(permission_decorator_1.PERMISSION_KEY, [
            context.getHandler(),
            context.getClass(),
        ]);
        if (!required)
        ...
        if (permissions[resource]?.[action] !== true) {
            throw new common_1.ForbiddenException(`Permission denied: requires ${resource}.${action}`);
        }
        return true;
    }
};
